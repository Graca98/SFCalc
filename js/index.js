...
window.onload = _e => {
  document.getElementById('calculator').onsubmit = _e => {
    ...
    for (const result in results) {
      ...
      let displayQuantity = quantity + ' (' + Math.floor(quantity / 64) + 'x 64';
      if (quantity % 64 > 0) {
        displayQuantity += ' + ' + (quantity % 64);
      }
      displayQuantity += ')';

      // Only add detailed display if total is greater than 64
      displayQuantity = quantity > 64 ? displayQuantity : quantity.toString();

      const disp = document.createElement('tr');
      disp.innerHTML = '<td class="' + color + '"><b>' + name + '</b></td><td>' + displayQuantity + '</td>';
